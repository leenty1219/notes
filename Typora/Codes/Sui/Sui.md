...
```objc
if ([SuiCloudAccountBookKit.sharedAccountBookKit canUseLocalDataModel]) {
        __weak typeof(self) weakSelf = self;
        [SharedKTManager.sharedManager  syncTransactionWithCompletion:^(SharedBoolean * _Nullable hasNewData, NSError * _Nullable error) {
            if (hasNewData && [hasNewData boolValue] == YES  && weakSelf) {
                [weakSelf reloadTransactionData];
            }
        }];
    }
```
